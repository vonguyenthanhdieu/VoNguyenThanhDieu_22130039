...
public class Order {
	...
	//Expand the implemented algorithms to sort an
	//array of order items in Order class 
	public static void insertionSort (OrderItem[] items) {
		for (int i =0; i< items.length; i++) {
			int j = i;
			OrderItem cur = items[i];
			while ( j>0 && items[j-1].compareTo(cur)>0) {
				items[j] = items[j-1];
				j--;
			}
			items[j] = cur;
	...
	public static void main(String[] args) {
		Product p1 = new Product("001", "Pepsi", 9.0, "Drink");
